...
for iGeneration = 1:numberOfGenerations
  disp(['Generation: ', num2str(iGeneration), ' Maximum fitness on training data: ', num2str(trainingMaximumOverall),...
    ' Maximum fitness on validation data: ', num2str(validationMaximumOverall)]);
  ...
  tempPopulation = InsertBestIndividual(tempPopulation, bestChromosome, numberOfElitismCopies);
  ...
  if (isTimeToValidate)
    ...
    for iPopulation = 1:populationSize
      ...
      if (validationFitness(iPopulation) > validationMaximumOverall)
        ...
        csvwrite('BestNetwork.csv', chromosome);
      end
    end
  end
